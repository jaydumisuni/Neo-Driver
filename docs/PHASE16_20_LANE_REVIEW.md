...
12. Native deployment completion waits for the operation, requires terminal `AsyncStatus::Completed`, then checks `DeploymentResult.ExtendedErrorCode` and surfaces deployment error text; canceled/error/nonterminal status cannot be reported as success.
13. After the removal call, Neo freshly observes the main package and all captured direct-dependency targets.
14. If post-write observation is unavailable, `machine_changed` is conservatively true; uncertainty cannot erase rollback obligation, and deterministic proof requires rollback plus preservation of the original observation failure in the returned diagnostic.
15. The single Debloat action records API outcome and observed `machine_changed` separately in the Phase 4 checkpoint.
16. Forward completion is determined by Phase 4 postcondition verification over only the declared main-package postcondition target; API success alone cannot complete the transaction.
...
19. Deterministic fake-host proof covers clean removal, both baseline-drift windows, partial failure after mutation, postcondition failure after dependency mutation, fail-closed post-write observation loss, no-change success, compound removal+rollback failure diagnostics, and dependency-preserving removal.
