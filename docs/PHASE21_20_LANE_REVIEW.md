...
15. **Durable session ownership** — resume state is append-only beneath Neo-owned `NeoData/sessions`, no-follow constrained, caller-bound, strictly decoded with unknown fields rejected, an `AlreadyExists` racing opener never claims creation/cleanup ownership, version publication is exclusive/no-replace, durable file bytes are synced before publication, and sudden-power-loss directory-entry durability is explicitly not claimed where the capability filesystem exposes no directory fsync.
