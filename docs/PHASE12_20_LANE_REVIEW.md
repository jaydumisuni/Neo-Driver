...
5. **Apply permission** — mutation requires `neo.tweaks.low-risk.apply` independently of prepare authority.
6. **Bounded request validation** — IDs/scopes are non-empty, bounded, control-character-free, and duplicate-free.
...
17. **Single-use mutation authority** — a validated apply removes the prepared service session before executor authority is exercised.
18. **Stable error taxonomy** — caller, permission, confirmation, plan/session, platform, no-change, and execution failures map to typed RPC error codes.
19. **No CLI mutation bypass** — Phase 12 adds no CLI dependency or public tweak-apply command.
20. **Frozen boundary + adversarial proof** — Decision 0012 and regressions prove unauthorized, unscoped, unconfirmed, mismatched, replayed, and failed-execution cases without live Registry writes.
