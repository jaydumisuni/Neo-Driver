...
checks = [
    ...
    ("09-phase5-pnp-semantics", has_all(MODEL, ("PnpStatusEvidence", "None => Ok(Self::NoProblem)", "Some(0)", "does not match the inherited Phase 5 problem-code evidence")) and has_all(TESTS, ("problem_code_zero_is_rejected_as_noncanonical_phase5_evidence", "explicit_pnp_status_must_match_device_problem_evidence"))),
    ("10-healthy-needs-no-problem-exact-package", "PnpStatusEvidence::NoProblem" in ASSESS and "DriverRepairState::Healthy" in ASSESS and "evidence.current_package.is_none()" in ASSESS and "healthy_exact_binding_requires_no_action" in TESTS),
    ...
    ("12-selection-only-for-real-problem", "PnpStatusEvidence::Problem { code } if !binding_present" in ASSESS and "PnpStatusEvidence::NoProblem if !binding_present" in ASSESS and "no_problem_without_binding_does_not_invent_driver_selection_need" in TESTS),
    ("13-disabled-code22-remains-read-only", "CM_PROB_DISABLED_CODE: u32 = 22" in MODEL and "DriverRepairState::Disabled" in ASSESS and has_all(TESTS, ("cm_prob_disabled_is_authoritative_when_generic_disabled_field_is_unavailable", "contradictory_disabled_evidence_fails_closed", "disabled_device_is_recorded_without_enable_authority"))),
    ...
    ("17-read-only-cli-surface", has_all(CLI, ("RepairCommand::Drivers", "inspect_windows_driver_repair", "DriverRepairEvidence::from_json_str", "Machine changes: none")) and "neo-driver-repair" in CLI_MANIFEST and '"pnp_status"' in FIXTURE and '"state": "no_problem"' in FIXTURE),
    ("18-adversarial-write-method-proof", has_all(TESTS, ("Phase 22 has no stage authority", "Phase 22 has no install authority", "Phase 22 has no rollback mutation authority", "Phase 22 has no Driver Store delete authority", "live_adapter_problem_path_invokes_only_inventory_and_exact_package_resolution"))),
