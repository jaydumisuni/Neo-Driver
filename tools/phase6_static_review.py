...
def review() -> list[Lane]:
    ...
    return [
        ...
        Lane(12, "double-hash-intake", contains_all(PRODUCTION, ["HashMismatch", "staged_hash", "promoted_hash", "expected_sha256"]), "pack intake validates source bytes, staging bytes, and promoted bytes"),
        Lane(13, "immutable-concurrent-promotion", contains_all(PRODUCTION, ["AlreadyPresent", "DestinationConflict", "ImportBusy", "create_new_file_nofollow", "create_new(true)"]) and "fs::rename" not in PRODUCTION and "promoted_pack_is_never_overwritten" in TESTS and "concurrent_same_pack_import_never_overwrites_or_leaves_staging_noise" in INTEGRATION_TESTS, "final promotion uses capability-relative create-new semantics: no overwrite, no bare rename, and concurrent imports leave no staging noise"),
        ...
        Lane(15, "cleanup-boundary", contains_all(PRODUCTION, ["open_existing_managed_child", '"staging"', "remove_dir_all(session.as_str())"]) and "remove_dir_all(&path)" not in PRODUCTION, "destructive cleanup is capability-relative to the retained staging directory rather than an absolute checked path"),
        Lane(16, "handle-nofollow-boundary", contains_all(PRODUCTION, ["cap_std::fs", "DirExt", "open_dir_nofollow", "OpenOptionsFollowExt", "FollowSymlinks::No", "open_absolute_dir_nofollow"]) and "ensure_directory_chain" not in PRODUCTION and "import_rejects_symlinked_package_directory_without_writing_outside" in INTEGRATION_TESTS and "audit_rejects_symlink_application_root" in INTEGRATION_TESTS, "application root and all managed writes use retained no-follow directory capabilities; symlinked package roots cannot redirect writes outside NeoData"),
        Lane(17, "existing-app-root", contains_all(PRODUCTION, ["open_absolute_dir_nofollow", "ApplicationRootUnavailable"]), "Neo opens the Builder/portable root as an existing no-follow capability rather than creating an arbitrary ambient root"),
