...
def review() -> list[Lane]:
    ...
    return [
        ...
        Lane(16, "rolled-back-proof", contains_all(TRANSACTION, ["verify_rollback", "require_successful_rollback_records", "TransactionStage::RolledBack"]), "ROLLED_BACK requires both rollback application and verification evidence"),
        Lane(17, "partial-failure", contains_all(TRANSACTION, ["successful_applied_ids", "all_reversible", "apply failure requires rollback"]), "partial apply failure only enters rollback when every changed action is reversible"),
