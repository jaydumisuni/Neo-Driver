...
# 1) Keep persisted legacy ApplyRecord semantics outcome-aware.
...
# 2) Derive reboot checkpoint kind from the enclosing persisted transaction stage.
...
# 3) Add transaction regressions for legacy records and reboot checkpoint rebinding.
...
text = tests.read_text(encoding="utf-8")
if text.count(marker) != 1:
    raise SystemExit("reboot checkpoint regression insertion anchor mismatch")
added = '''#[test]
...
# 4) Reject lexical parent traversal during direct plan/session deserialization.
...
# 5) Host exposes the actual Windows build so request/plan build cannot be caller fiction.
...
# 6) Stage failure recovery remains conservative even if package identity recovery fails.
...
# 7) Windows source: trusted directory, canonical DEVPROPKEY and trusted build query.
...
# 8) Fake host + adversarial regressions.
...
if text.count(marker) != 1:
    raise SystemExit("driverstore review regression anchor mismatch")
added = '''#[test]
...
# 9) Static gate scans production source separately from tests and binds new laws.
...
# 10) Add Registry feature for trusted host build query.
